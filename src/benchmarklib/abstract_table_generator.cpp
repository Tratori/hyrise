...
void AbstractTableGenerator::generate_and_store() {
  ...
  if (_benchmark_config->relocate_numa) {
    ...
    std::cout << "Relocate data using default_memory_resource " << std::endl; // << num_nodes << " nodes" << std::endl;
    ...
    for (auto& [table_name, table_info] : table_info_by_name) {
      ...
      for (auto chunk_id = ChunkID{0}; chunk_id < chunk_count; ++chunk_id) {
        auto migrate_job = [&, chunk_id]() {
          const auto& chunk = table->get_chunk(chunk_id);
          chunk->migrate(boost::container::pmr::get_default_resource());
        };
  ...
  std::cout << "Relocation took:  " << global_timer.lap_formatted() << std::endl; 

  std::cout << "Segment-Locations after relocation" << std::endl;
  print_numa_location_of_segments(table_info_by_name);
